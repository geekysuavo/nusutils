...
int main (int argc, char **argv) {
   ...
   *
   * the JITUTIL_DIMS_MIN and JITUTIL_DIMS_MAX preprocessor definitions,
   * located in jitutil.h, are used to soft-limit the number of dimensions
   * that the program will build sampling schedules upon.
   */
  if (D < JITUTIL_DIMS_MIN || D > JITUTIL_DIMS_MAX) {
    /* output a usage statement and return failure. */
    fprintf(stderr, JITUTIL_USAGE, argv[0]);
    return 1;
  }
